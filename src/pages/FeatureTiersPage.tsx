    ...
    >
      <header className="mb-20 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mb-16">
          ...
          <button 
            ...
            className="px-12 py-6 bg-black/5 text-black font-bold text-xl rounded-full hover:bg-black/10 transition-all border border-black/10 flex items-center gap-3"
          >
            Explore Plans <span className="text-2xl">↓</span>
          </button>
        </div>
      </header>
      ...
      <div id="pricing-grid" className="grid grid-cols-1 md:grid-cols-2 max-w-4xl mx-auto gap-8 mb-20">
        {PRICING_TIERS.map((tier) => (
          <div 
            key={tier.id} 
            className={`relative p-10 rounded-[3rem] border border-black/5 flex flex-col transition-all duration-300 hover:scale-[1.02] ${
              tier.id === 'creator' ? 'bg-black text-white' : 'bg-white text-black'
            } ${tier.popular ? 'shadow-2xl border-[#EFBB76]/50' : 'shadow-sm'}`}
          >
            {tier.popular && (
            ...
            <div className="space-y-4">
              ...
              >
                ...
                {tier.id === 'free' && <ArrowRight className="w-3 h-3" />}
              </button>
              {tier.id === 'free' && (
      ...
      <div className="bg-black/5 p-12 rounded-[3rem] border border-black/10 text-center space-y-6">
        <h3 className="text-sm font-bold uppercase tracking-widest">For Studios, Developers & Procurement Teams</h3>
        <p className="text-xs text-black/40 max-w-xl mx-auto leading-relaxed">Access custom workflows, private production networks, and project-based pricing tools.</p>
        <button 
          onClick={() => window.open('mailto:hello@opul.mkt')}
          className="text-[10px] font-black uppercase tracking-widest text-[#EFBB76] hover:text-[#DBA762] transition-colors flex items-center justify-center gap-2 mx-auto"
        >
